...
const PaymentModal = ({ shipment, onClose, onPaymentComplete }) => {
  ...
  // In the handleSubmit function, update the logic to handle partial payments
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const transactionsResponse = await api.get(`/transactions/request/${shipment.id}`);
      ...
      if (transactionsResponse.data.data && transactionsResponse.data.data.length > 0) {
        ...
        response = await api.put(`/transactions/${transactionId}/payment`, formData);
      } else {
        ...
        {isLoading ? (
          ...
          <div className="mt-6 flex justify-end space-x-3">
            <button
              ...
              disabled={isSubmitting || !formData.payment_amount || parseFloat(formData.payment_amount) <= 0 || parseFloat(formData.payment_amount) > remainingAmount}
            >
              Cancel
            </button>
            <button
